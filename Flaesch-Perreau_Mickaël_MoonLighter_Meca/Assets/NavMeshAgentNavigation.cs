...
public class NavMeshAgentNavigation : MonoBehaviour
{
    [SerializeField] private Transform Hs_H_Door;
    [SerializeField] private Transform Hs_S_Door2;

    [SerializeField] private Transform CaisseWaitingPos;
    [SerializeField] private Transform CaisseWaitingPos2;
    [SerializeField] private Transform CaisseWaitingPos3;
    [SerializeField] private Transform CaisseWaitingPos4;

    [SerializeField] private Transform SavePos1;
    [SerializeField] private Transform SavePos2;
    [SerializeField] private Transform SavePos3;
    [SerializeField] private Transform SavePos4;
    [SerializeField] private Transform SavePos5;
    [SerializeField] private Transform SavePos6;
    [SerializeField] private Transform SavePos7;
    [SerializeField] private Transform SavePos8;
    [SerializeField] private Transform SavePos9;
    [SerializeField] private Transform SavePos10;
    [SerializeField] private Transform SavePos11;
    [SerializeField] private Transform SavePos12;
    ...
    private void EnteringFirstDeplacement()
    {
        ...
        {
            step1 = true;
            Agent.destination = Hs_H_Door.position;
    ...
    private void SearchingObjet()
    {
        if (startingSearchingObjet == true & leavingShop == false & goingToPay == false)
        {
            ...
            CheckParcour(1, 2, ref objet1Checked, ref GameManager.Instance.objet1OnSelling, InputFields.Instance.price1,
                    SavePos4.position, SavePos6.position, SavePos8.position, SavePos8.position,
                    SavePos10.position, SavePos8.position, SavePos6.position, SavePos6.position,
            ...
            CheckParcour(2, 3, ref objet2Checked, ref GameManager.Instance.objet2OnSelling, InputFields.Instance.price2,
                    SavePos3.position, SavePos3.position, SavePos7.position, SavePos5.position,
                    SavePos1.position, SavePos3.position, SavePos5.position, SavePos5.position,
            ...
            CheckParcour(3, 4, ref objet3Checked, ref GameManager.Instance.objet3OnSelling, InputFields.Instance.price3,
                    SavePos1.position, SavePos11.position, SavePos9.position, SavePos9.position,
                    SavePos11.position, SavePos11.position, SavePos7.position, SavePos9.position,
            ...
            CheckParcour(4, 1, ref objet4Checked, ref GameManager.Instance.objet4OnSelling, InputFields.Instance.price4,
                    SavePos2.position, SavePos2.position, SavePos10.position, SavePos12.position,
                    SavePos12.position, SavePos12.position, SavePos4.position, SavePos2.position,
        ...
        ref int SPID_1, ref int SPID_2, ref int SPID_3, ref int SPID_4, ref bool pSlotEmpty, ref string pSlotID)
    {
        ...
        if (clientIDObjetSearching == IDObjetSearching & objetInHand == false)
        {
            ...
            if (ObjetCheked == false)
            {
                ...
                {
                    step7 = true;
                    if (SPID_1 != personalID) Agent.destination = SavePos1;
                    else Agent.destination = SavePos1_2;
                }
                if (transform.position.x <= -15f)
                {
                    step8 = true;
                    if (SPID_2 != personalID) Agent.destination = SavePos2;
                    else Agent.destination = SavePos2_2;
                }
                if (transform.position.x <= -9 & transform.position.x >= -15f & transform.position.z <= -3.7f)
                {
                    step9 = true;
                    if (SPID_3 != personalID) Agent.destination = SavePos3;
                    else Agent.destination = SavePos3_2;
                }
                if (transform.position.x <= -9 & transform.position.x >= -15f & transform.position.z >= 2.3f)
                {
                    step10 = true;
                    if (SPID_4 != personalID) Agent.destination = SavePos4;
                    else Agent.destination = SavePos4_2;
                }
    ...
    private void ActualyLeavingShop()
    {
        if (leavingShop == true)
        {
            ...
            if (transform.position.x <= -15f & transform.position.z < 2.3f & GameManager.Instance.sP10ID != personalID)
                Agent.destination = SavePos10.position;

            else if (transform.position.x <= -9 & transform.position.x >= -15f & transform.position.z >= 2.3f
                & GameManager.Instance.sP4ID != personalID) Agent.destination = SavePos4.position;

            else Agent.destination = Hs_S_Door2.position;
            ...
            & GameManager.Instance.cWP3ID != personalID & GameManager.Instance.cWP4ID != personalID)
        {
            ...
            else
            {
                ...
                if (GameManager.Instance.cWP1Reaching == 0 || GameManager.Instance.cWP1Reaching == personalID)
                    ReachingWaitingFill(ref GameManager.Instance.cWP1Reaching, CaisseWaitingPos.position, ref stepD3);
                else if (GameManager.Instance.cWP2Reaching == 0 || GameManager.Instance.cWP2Reaching == personalID)
                    ReachingWaitingFill(ref GameManager.Instance.cWP2Reaching, CaisseWaitingPos2.position, ref stepD4);
                else if (GameManager.Instance.cWP3Reaching == 0 || GameManager.Instance.cWP3Reaching == personalID)
                    ReachingWaitingFill(ref GameManager.Instance.cWP3Reaching, CaisseWaitingPos3.position, ref stepD5);
                else if (GameManager.Instance.cWP4Reaching == 0 || GameManager.Instance.cWP4Reaching == personalID)
                    ReachingWaitingFill(ref GameManager.Instance.cWP4Reaching, CaisseWaitingPos4.position, ref stepD6);
            }
        }
    }
